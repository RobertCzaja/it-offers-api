...
public class JustJoinItRawOfferBuilder {
  ...
  public JustJoinItRawOfferBuilder salary(
      String type, String currency, Integer from, Integer to, Double fromPln, Double toPln) {
    ArrayList<LinkedHashMap> employmentTypes =
        (ArrayList<LinkedHashMap>) data.get("employmentTypes");
  ...
  public JustJoinItRawOffer build() {
    JustJoinItRawOffer rawOffer =
        new JustJoinItRawOffer(
            UUID.randomUUID(), "technologyThatDoesNotMatter", data, LocalDateTime.now());
    cleanState();
    return rawOffer;
  }
}
